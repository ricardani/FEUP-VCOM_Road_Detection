...
int main(int argc, char* argv[]) {
	...
	if (option == 1)
	{
		image = cv::imread("road6.jpg", 1);
		if (!image.data)
			return 0;

		// Read input image
		Mat thresh, final_mask, image_final;
		final_mask = Mat::zeros(image.size(), CV_8UC1);

		//convert grey
		cvtColor(image, image_gray, CV_BGR2GRAY);
		thresh = roadSegmentation(image_gray);
		thresh = CannyHough(image_gray, thresh);
		...
		image_final = fillRoad(thresh, image);

		namedWindow("start", WINDOW_KEEPRATIO);
		imshow("start", image);

		namedWindow("image", WINDOW_AUTOSIZE);
		imshow("image", image_final);
